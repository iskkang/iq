 ...
 *
 * **캡처에 나오는 SKU 는 랜딩 표와 같은 품목이어야 한다** (백팩·머그·팬).
 * 방문자가 표에서 본 숫자를 캡처에서 다시 발견하는 순간이 "진짜 계산엔진이구나"를
 * 만든다. 다른 품목이면 그 효과가 사라진다. 가입 즉시 생성되는 선적재가 이미 같은
 * 목록이고 tests/repo.sample.test.ts 가 그 일치를 못으로 박아뒀으므로, CSV 를 따로
 * 업로드하지 말고 **그 선적을 열기만 하면 된다.**
 *
