...
| 명령 | 내용 |
|---|---|
| `npm run test` | 단위 테스트 110건 (§4 계산식 + 골든 10건 + CSV 파싱 + 301 원산지 스코핑 + 분류 파이프라인) |
...
| 2 | CSV 500행 → 리포트 3분 이내 | ❌ **실패 — 실측 321s** (배치 45.9s × 7웨이브, 동시성 8). 결정론 구간은 19ms, 전부 LLM 왕복이다 |
...
| §2-4 원산지 스코핑 | ✅ 통과 |
| §6-2 500 SKU 3분 | ❌ **실패 — 321s** (v2 파이프라인이 배치당 호출 1→5회). 아래 참조 |
...
3. **temperature 0** + 정규화 해시 캐시(`classification_cache`) — 같은 CSV 를 다시 올리면 같은 결과
4. **auto_confirmed = k=3 만장일치 AND 원장 base MFN 실존.** confidence 는 판정에서 빠지고 참고 표기
...
`npm run bench -- --sample=10` 결과:

| 항목 | 값 |
|---|---|
| SKU당 API 비용 | **$0.0568** (sonnet 2단계×k=3 + haiku 교차검증 1회) |
| 500 SKU | $28.39 |
| 배치(10 SKU) wall-time | 45.9s → 50배치 ÷ 동시8 = **321s** (기준 180s) ❌ |

플랜 마진 (캐시 0% 최악):

| 플랜 | 가격 | SKU | API 비용 | 마진 |
|---|---|---|---|---|
| Starter | $29 | 50 | $2.84 | 90.2% |
| Growth | $79 | 500 | $28.39 | **64.1%** ⚠️ |
| Pro | $149 | 2000 | $113.56 | **23.8%** ⚠️ |

**$29/50SKU 플랜은 안전하다 (90%). 깨지는 쪽은 Growth·Pro 다.** 분류 결과 캐시
(`classification_cache`, 정규화 해시)가 방어선이며 히트율 50%면 Growth 82%로 회복된다.
비용 드라이버는 stage-B 프롬프트의 보기 목록(호당 최대 60줄 × 최대 3호 × 10 SKU ≈ 38k 토큰/호출)이라,
`MAX_LINES_PER_HEADING` 축소가 시간·비용 양쪽에 듣지만 정답 라인이 잘릴 위험이 있어 미적용으로 뒀다.
