...
| 명령 | 내용 |
|---|---|
| `npm run test` | 단위 테스트 98건 (§4 계산식 + 골든 10건 + CSV 파싱 + 301 원산지 스코핑 + 분류 파이프라인) |
...
| 2 | CSV 500행 → 리포트 3분 이내 | ⚠️ **재측정 필요** — v2 파이프라인은 배치당 LLM 호출이 1 → 최대 6회(2단계×k=3). 배치 동시성을 4→8 로 올려 ≈112s 로 추정하나 실측 안 함 |
...
| 검증 | 결과 |
|---|---|
| §1 HTS 분류 | `claude-sonnet-4-6` **8.0/10 통과** (5회 8–8) · `claude-haiku-4-5` 5.2/10 미달 |
| §2-1·2 세율 | 부분 통과 — base MFN 17,633행 USITC 공식 적재 완료. **301·IEEPA 12행은 여전히 SAMPLE** |
| §2-3 계산 정확도 | ✅ 통과 — 배부 보존·MPF 캡 3경로 |
| §2-4 원산지 스코핑 | ✅ 통과 |
| §3 E2E | ⛔ 미집행 — UI 수동 수행 필요 |

**광고 집행은 아직 불가**: 오답의 needs_review 격리율이 0% 다. temperature 0 으로 고정하면
k=3 투표가 거의 항상 만장일치가 되어(94~100%) 안전판이 사실상 상수가 된다 — 재현성(요구사항 2)과
만장일치 안전판(요구사항 3)이 서로를 무력화한다. 자세한 진단·개선안은 [test-results-v3.md](test-results-v3.md) 판정 절.
